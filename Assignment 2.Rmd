...
```{r}
# #put data into tidy format and tokenize to bigrams
df_bigrams <- sona %>%
  unnest_tokens(bigram, speech, token = 'ngrams', n = 2, to_lower = T) %>%
select(bigram, everything())

#strip all punctuation and numeric characters from each bigram
df_stripped_bigrams = df_bigrams%>%
  mutate(bigram= str_replace_all(bigram, "[[:punct:]]", "")) #remove punctuation

  
# separate the bigrams 
df_separated_bigrams <- df_stripped_bigrams %>%
...
#remove word 1 and word 2 column from original bigrams separtaed data frame
df_separated_bigrams = df_separated_bigrams %>% select(filename, year, president_13, date)

#combine sentiments for word1 and word2 into 1 dataframe
bigrams_sentiment = cbind(df_separated_bigrams, bigrams_word1_sentiment , bigrams_word2_sentiment )

###############################################################################

#Find negation words - we find none
#negation words
negation_words <- c('not', 'no', 'never', 'without')

# show a few
filter(bigrams_sentiment, word1 %in% negation_words) %>% 
  head(10) %>% select(year, word1, word2, word1_sentiment, word2_sentiment)
 
################################################################################

## replacing NA with 0 just gives lots of neutral sentiments
#let sentiment of word = 0 if sentiment = NA
# bigrams_sentiment$word1_sentiment[is.na(bigrams_sentiment[, 'word1_sentiment'])] = 0 #sentiment for word 1
# bigrams_sentiment$word2_sentiment[is.na(bigrams_sentiment[, 'word2_sentiment'])] = 0 #sentiment for word 2

#remove bigrams where either word 1 or word 2 or both have sentiment == NA ---> not useful
# bigrams_sentiment = bigrams_sentiment[!is.na(bigrams_sentiment$word1_sentiment) & !is.na(bigrams_sentiment$word2_sentiment), ]
###############################################################################
...
# 8. Total sentiment over all years, all speeches and across all presidents

```{r}
#negative, if total sentiment < 0
#positive, else if total sentiment > 0
#neutral if total sentiment = 0
...
#replace total numeric sentiment with total logical sentiment
...
#look at negative and positive sentiment
df_bigrams%>% select(bigram, sentiment) %>% filter(sentiment == "negative" | sentiment == "positive")
