...
Sentiment analysis and topic modelling are used in a variety of fields, with application in financial market prediction, business review analyses, and political science research.

Bhardwaj et al. (2015)1 describes a system in which sentiment analysis is applied to stock indices that act as indicators for overall stock market performance. Another paper by Rao Srivastava (2012) assessed the association between tweet sentiments and market performance, by assessing tweet and market data from 14 companies. The authors used a Naïve Bayesian classifier and found that the polarity of sentiments significantly influenced investment opportunities.

Businesses may also benefit from sentiment analysis by assessing customer preferences and market trends. These organizations perform sentiment analysis on customer review data with the aim of improving product- or service quality. Singla et al. (2017) performed sentiment analysis on Amazon phone review data. The authors developed a support vector machine (SVM) model for sentiment classification and found that most customers preferred Samsung phones.

Sentiment analysis has also been used in political science research, where these studies use data obtained from social media sites. Singh et al. (2017) proposed a method to assess public opinion on demonetization, an experiment particularly relevant in India, where the government demonetized some banknotes.

In the past, public opinion was collected using interviews and surveys, however, in the modern era of social media, there is a multitude of shared opinions expressed on online platforms such as Twitter. Researchers are now interested in how news media can outline a campaign or a public figure and how this affects public opinion.

Researchers often perform sentiment analysis and topic modelling in tandem to discern public sentiment, as well as recurring topics (Guo et al., 2016).

Topic modelling is an unsupervised machine learning technique that uncovers abstract 'topics' within a (collection of) document(s). This approach views each document as an unordered "bag of words", with words appearing at varying frequencies. The model then calculates the topical weights in the document based on the frequency of words related to those topics. We can then identify the central theme of the document.

The most widely used topic model is Latent Dirichlet Allocation (LDA) which assumes that the proportion of topics in a document is sampled from a Dirichlet distribution (LDA; Blei, Ng, & Jordan, 2003).

LDA models are usually applied to research that uses well-constructed text documents such as academic journal articles. In contrast, LDA models may not be effective when using social media data (Hong & Davison, 2010; Tang, Zhang, & Mei, 2013). For example, tweets are usually unstructured, contain abbreviations and misspelt words. Despite this limitation, many researchers have used social media data due to the large volume of freely available, public sentiment.

A study by Biraj Dahal et al (2019) assessed public opinion on climate change, by performing sentiment analysis and LDA on geotagged tweets. The researchers compared themes and sentiment concerning climate change between different countries. Sentiment analysis suggested that the overall debate is negative, especially when Twitter users react to political news. On the other hand, topic modelling suggested polarity in topics, with topics such as "carbon footprint" and "weather/belief". The authors also found that the USA had a negative sentiment towards climate change, with topics such as "politics/hoax" as central themes.
